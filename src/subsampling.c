...
/* Caller-owned work buffers for conv2d_s2 (streaming: carved once per stream).
 * Every field may be NULL/0, in which case that buffer is malloc'd per call —
 * which is what the offline path does. The arithmetic is the same either way. */
typedef struct {
    float *im2col;          /* >= 9 * To * Fo floats (stage 0 full conv) */
    float *pad;             /* >= pad_slices * pad_stride floats (depthwise) */
    size_t pad_stride;
    int pad_slices;
} ss_scratch;
...
typedef struct {
    ...
    float *pad_base;        /* NULL = each worker mallocs its own pad */
    size_t pad_stride;
    atomic_int failed;
} dw_par;
...
static void dw_slice_worker(void *ctx, int wslice) {
    ...
    float *pad = dp->pad_base ? dp->pad_base + (size_t)wslice * dp->pad_stride
                              : malloc((size_t)Tp * (size_t)Fp2 * sizeof(float));
...
static void conv2d_s2(const float *x, int C_in, int T, int F, int pl_t, int pr_t,
    ...
    *To_ = To; *Fo_ = Fo;
    ...
    if (!depthwise && C_in == 1) {
        ...
        float *P = (sc && sc->im2col) ? sc->im2col : malloc(9u * S * sizeof(float));
        if (P) {
    ...
    if (depthwise) {
        ...
        /* caller scratch only when it covers every slice: the slice count (and
         * therefore the arithmetic) is decided exactly as before */
        if (sc && sc->pad && dp.slices <= sc->pad_slices &&
            (size_t)Tp * (size_t)dp.Fp2 <= sc->pad_stride) {
            dp.pad_base = sc->pad;
            dp.pad_stride = sc->pad_stride;
        }
...
int mynah_asr_ss_stream_init(mynah_asr_ss_stream *sst, const mynah_asr_subsampling *ss,
    ...
    if (max_n_mel <= 0) return 0;

    /* per-step scratch, carved out of ONE allocation (same bounds the step used
     * to malloc per chunk, taken at the maximum chunk size). The first stage is
     * the largest one, so every later stage fits in the same buffers. */
    const size_t C = (size_t)ss->channels;
    const size_t cap_t = (size_t)max_n_mel / 2 + 2, cap_f = (size_t)n_mels / 2 + 2;
    const size_t SA = C * cap_t * cap_f;
    size_t xp = C * (cap_t + 3) * cap_f;                     /* stages 1..2 */
    const size_t xp0 = ((size_t)max_n_mel + 3) * (size_t)n_mels;  /* stage 0 (C_in = 1) */
    if (xp0 > xp) xp = xp0;
    const size_t im2col = 9u * cap_t * cap_f;
    const int slices = mynah_asr_num_threads();
    const size_t pad_stride = (cap_t + 3) * (cap_f + 3);
    const size_t sz = SA                      /* sa                      */
                    + SA                      /* sb (sflat aliases it)   */
                    + xp                      /* sxp                     */
                    + im2col                  /* sim2col                 */
                    + (size_t)slices * pad_stride; /* spad               */
    sst->scr = malloc(sz * sizeof(float));
    if (!sst->scr) return -1;
    float *p = sst->scr;
    sst->sa = p; p += SA;
    sst->sb = p; p += SA;
    sst->sflat = sst->sb;
    sst->sxp = p; p += xp;
    sst->sim2col = p; p += im2col;
    sst->spad = p;
    sst->pad_slices = slices;
    sst->pad_stride = pad_stride;
    return 0;
}
...
static void stream_stage(const float *x, int C_in, int T, int F, int first, int last,
                         ...
                         int depthwise, float *out, int *To_, int *Fo_,
                         float *xp_scr, const ss_scratch *sc) {
    ...
    float *xp = xp_scr ? xp_scr
                       : malloc((size_t)C_in * (size_t)Tp * (size_t)F * sizeof(float));
    ...
    conv2d_s2(xp, C_in, Tp, F, 0, 0, 2, 1, w, b, C_out, depthwise, out, To_, Fo_, sc);
    if (!xp_scr) free(xp);
}
...
int mynah_asr_ss_stream_step(const mynah_asr_subsampling *ss, mynah_asr_ss_stream *sst,
    ...
    const int C = ss->channels;
    int To = 0, Fo = 0;
    const int owned = sst->scr == NULL;
    const int cap_t = n_mel / 2 + 2, cap_f = n_mels / 2 + 2;
    const ss_scratch sc = {.im2col = sst->sim2col, .pad = sst->spad,
                           .pad_stride = sst->pad_stride, .pad_slices = sst->pad_slices};
    float *a = owned ? malloc((size_t)C * (size_t)cap_t * (size_t)cap_f * sizeof(float))
                     : sst->sa;
    float *bbuf = owned ? malloc((size_t)C * (size_t)cap_t * (size_t)cap_f * sizeof(float))
                        : sst->sb;
    if (!a || !bbuf) { if (owned) { free(a); free(bbuf); } return -1; }

    stream_stage(mel, 1, n_mel, n_mels, sst->first, is_last, sst->cache[0],
                 (const float *)ss->conv_in_w->data, (const float *)ss->conv_in_b->data,
                 C, 0, a, &To, &Fo, sst->sxp, &sc);
    relu_inplace(a, (size_t)C * (size_t)To * (size_t)Fo);
    ...
    const int CF = C * Fo;
    /* sflat aliases sb, whose depthwise output the pointwise GEMM above has
     * already consumed into a */
    float *flat = owned ? malloc((size_t)To * (size_t)CF * sizeof(float)) : sst->sflat;
    if (!flat) { if (owned) { free(a); free(bbuf); } return -1; }
    for (int t = 0; t < To; t++)
        for (int c = 0; c < C; c++)
            memcpy(flat + (size_t)t * (size_t)CF + (size_t)c * (size_t)Fo,
                   a + ((size_t)c * (size_t)To + (size_t)t) * (size_t)Fo,
    ...
    if (owned) { free(a); free(bbuf); free(flat); }
    return To;
}
