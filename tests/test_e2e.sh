...
if [ "$ENGINE" = "canary-aed" ] && [ -f "$LID_DIR/mynah.json" ]; then
    ...
    if [ -f samples/ja/fleurs_1521.wav ]; then
        err=$(./mynah-asr transcribe -m "$MODEL_DIR" -i samples/ja/fleurs_1521.wav \
              --lang auto --lid-model "$LID_DIR" --target-lang en 2>&1 >/dev/null)
        rc=$?
        case "$rc:$err" in
            0:*) echo "e2e lid-unsupported FAIL: exited 0 on ja"; fail=1 ;;
            *"is not supported by the model"*) echo "e2e lid-unsupported OK (ja rejected)" ;;
            *) echo "e2e lid-unsupported FAIL: $err"; fail=1 ;;
        esac
    fi
fi
