...
struct legacy_asr_safetensors {
    void *map;
    size_t map_len;
    cJSON *header;         /* keeps the name strings alive */
    legacy_asr_tensor *tensors;
    size_t n_tensors;
    legacy_asr_gguf *gguf;      /* alternative container: if set, tensors point there */
};

static int dtype_of(const char *s, legacy_asr_dtype *out) {
    if (strcmp(s, "F32") == 0) { *out = LEGACY_ASR_DT_F32; return 0; }
    if (strcmp(s, "F64") == 0) { *out = LEGACY_ASR_DT_F64; return 0; }
    if (strcmp(s, "BF16") == 0) { *out = LEGACY_ASR_DT_BF16; return 0; }
    if (strcmp(s, "F16") == 0) { *out = LEGACY_ASR_DT_F16; return 0; }
    if (strcmp(s, "I8") == 0) { *out = LEGACY_ASR_DT_I8; return 0; }
    if (strcmp(s, "U8") == 0) { *out = LEGACY_ASR_DT_U8; return 0; }
    if (strcmp(s, "I64") == 0) { *out = LEGACY_ASR_DT_I64; return 0; } /* BatchNorm num_batches_tracked */
    return -1;
}

legacy_asr_safetensors *legacy_asr_st_open_quiet(const char *path) {
    int fd = open(path, O_RDONLY);
    if (fd < 0) return NULL;
    close(fd);
    return legacy_asr_st_open(path);
}
...
legacy_asr_safetensors *legacy_asr_st_open(const char *path) {
    ...
    if (pread(fd, magic, 4, 0) == 4 && memcmp(magic, "GGUF", 4) == 0) {
        close(fd);
        legacy_asr_gguf *g = legacy_asr_gguf_open(path);
        if (!g) return NULL;
        legacy_asr_safetensors *st = calloc(1, sizeof(*st));
        if (!st) { legacy_asr_gguf_close(g); return NULL; }
        st->gguf = g;
        st->tensors = (legacy_asr_tensor *)legacy_asr_gguf_tensors(g, &st->n_tensors);
        return st;
    }
...
size_t legacy_asr_st_count(const legacy_asr_safetensors *st) { return st->n_tensors; }
