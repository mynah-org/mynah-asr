...
- [ ] M-2 The model constants that should be config reads: CTC blank (`src/decoder_ctc.c`, `V-1` while the pack carries `decoder.blank_id`), the VAD sample rate, the subsampling factor in the three chunk-geometry sites, the server's six `16000` literals
