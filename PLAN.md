...
- [x] S1-6 BLAS leaves the Linux worker: `src/sgemm.c` (five families, reduction over k never split, row block rounded to the micro-kernel, so the result is byte-identical at 1/2/4/8 threads), `mynah_asr_gemm_f32`/`_gemv_f32` in `src/backend.c` as the ONE seam every f32 GEMM goes through — no file outside it includes `cblas.h` — and `make BLAS=none|openblas|accelerate` with `none` the **Linux default**, `openblas` kept as the comparison arm in its own CI job. `tests/test_sgemm.c` gates every family against the always-compiled scalar reference and refuses if a family never ran; the seam itself is checked in all three builds. On this M1 `tests/test_streaming.c` is byte-identical between the two builds and every `tests/test_stream_batch.c` identity gate passes in both; the one transcript difference is the es-ES clip the batch test already prints as unstable in BOTH builds. **Accelerate is faster on macOS by enough to break a 4-stream real-time envelope that it keeps GOOD, and the note says so plainly**; the decision is ownership on Linux, not speed on macOS. **Every timing number is deferred to the Linux box (S1-6a): none was taken here.** → [`.work/threadpool-and-lane.md`](.work/threadpool-and-lane.md)
- [ ] S1-6a Linux only, now the REMAINING half of S1-6: `BLAS=openblas` vs `BLAS=none` on the Axion and on x86 — RTF, the batched step table, thread count inside a pinned prefork worker (2T vs T), and the OpenBLAS f32 row-stability gate that decides `MYNAH_ASR_BATCH_F32` there. No Linux number for our own sgemm exists yet → [`.work/threadpool-and-lane.md`](.work/threadpool-and-lane.md)
