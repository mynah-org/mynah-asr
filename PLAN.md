...
- [~] S1-6 BLAS leaves the worker: `src/sgemm.c` is ours, deterministic across thread counts and gated (`tests/test_sgemm`), behind one seam, with `BLAS=none|openblas|accelerate` and CI running `none` on both Linux arches. The Linux DEFAULT is still `openblas`: the A/B that owns that decision has never been run (S1-6a, `.work/box-day-plan.md` step 4); on the M1 Accelerate beats `own` by enough to break four real-time streams → [`.work/threadpool-and-lane.md`](.work/threadpool-and-lane.md)
