...
- [ ] S8-1 **Calibrate, do not assume**: the advisor runs the step table at B=1,2,4,8 on the actual box and fits `a` and `b` (discarding B=1 as warm-up), because a prediction from a spec sheet is a horoscope. ~30 s
- [ ] S8-2 Predict per quantisation and per lookahead with the arithmetic shown and labelled a prediction: `B_max = (rho*P - a) / b` per worker, `W * B_max` for the fleet. Lookahead is the biggest single lever — 3 -> 6 nearly doubles capacity for 240 ms of latency — and belongs in the output as an explicit trade
- [ ] S8-3 Recommend a ready-to-paste command line: W x T (starting WIDE, see S8-5), `--cap`, `--quant`, the lookahead that meets a stated latency budget, and the backlog/descriptor limits S7 already derives. Refuse when an input is missing rather than substituting a default
- [ ] S8-4 Close the loop: a WAVE/SOAK artifact carries the measured capacity so the next run prints `predicted 49, measured 38`. A predictor that never learns it was wrong is a horoscope
- [ ] S8-5 Test the two predictions the cost model makes, each one experiment: (a) `a` is weight traffic, so it barely improves from 8 to 32 threads and nearly halves at int4; (b) because every worker pays `a` again, capacity is maximised by FEW WIDE workers — the sweep should start at 2x16, not 8x4. If capacity is flat in W, the model is wrong and S8-3's advice changes
