...
- [ ] S6-5 Quality, as opposed to speed: the load harness must REFUSE to print a KPI table when the batched-identity comparison fails (the sibling's `serve_identity_gate.py` rule — a number from a run whose transcripts moved is worse than no number); a CER drift track against the committed `samples/` bank; and named drift thresholds in the soak the way the sibling names its 30%/20%/resource-growth gates
...
- [ ] S7-1 `listen(srv, 64)` in `server/main.c`: 100 clients arriving together overflow the backlog and the kernel drops SYNs, so the run measures the accept queue. Raise it and put it in the admission banner with the rest of the ladder
- [ ] S7-2 `RLIMIT_NOFILE` is whatever was inherited: 100 streams plus writer descriptors plus the metrics port is a silent ceiling. Raise it at start-up and print what it became
