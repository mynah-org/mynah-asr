 ...
 * processes that run inference. Lifted from mynah-tts (server/prefork.{c,h} at
 * 8aa714a, 2026-09-18) with the mynah_asr_ prefix; the decoder lane and the
 * cost map it referenced do not exist here yet. Design: .work/serving-v2-design.md.
 *
 ...
 *   - W too large: W copies of every per-process cache. The memory-mapped model
 *     is shared (see below), but the quantized-weight and codec-filter caches
 *     each worker builds on its first inference are NOT. Symptom: RSS grows
 ...
 *   worker, no prefork, sweep MYNAH_ASR_THREADS over 1,2,4,8,16,32 and record RTF
 *   at concurrency 1 and at concurrency equal to --max-batch:
 *
 *     for t in 1 2 4 8 16 32; do
 *       MYNAH_ASR_THREADS=$t python3 tools/bench/stream_load.py --streams N MODEL_DIR=... ARGS="--levels 1,4"
 *     done
 *
 ...
 *
 *     for w in 4 8 16; do
 *       ./mynah-asr-server -m PACK --prefork $w --prefork-threads $((C/w)) &
 *       python3 tools/bench/stream_load.py --streams N MODEL_DIR=MODEL ARGS="--levels $((w)),$((2*w)),$((4*w))"
 *     done
 *
 ...
 *   is more than about (model size + W * per-worker cache), a cache you thought
 *   was shared is not.
 *
 * A note on what the fork does and does not share. The children are forked
 * AFTER the model is open, so the memory-mapped weights are one physical copy
 * for the whole tree. They are forked BEFORE any inference, which is required
 * for correctness (a fork from a thread other than main, or after a inference
 ...
 *      moment the parent discovered the wait, not a deadline served a request at
 ...
 *      mynah_asr_prefork_service_cap_ms(); the parent additionally watches the
 ...
 * isolation). The full table is .work/serving-v2-design.md §8. When the machine is
 * full the answer is to REFUSE, not to get clever.
 */
#ifndef MYNAH_ASR_SERVER_PREFORK_H
#define MYNAH_ASR_SERVER_PREFORK_H

#include <signal.h>
#include <stdio.h>

typedef enum {
    MYNAH_ASR_PREFORK_DISABLED,     /* not requested: the caller serves in-process */
    MYNAH_ASR_PREFORK_CHILD,        /* we are a worker: serve from *chan_fd */
    MYNAH_ASR_PREFORK_PARENT_DONE,  /* the router ran to completion: exit */
    MYNAH_ASR_PREFORK_ERROR         /* could not set up: the caller must fail */
} mynah_asr_prefork_role;
...
typedef enum {
    ...
    MYNAH_ASR_PREFORK_REFUSE_AT_CAPACITY = 0,
    ...
    MYNAH_ASR_PREFORK_REFUSE_QUEUED_TOO_LONG,
    ...
    MYNAH_ASR_PREFORK_REFUSE_SERVICE_CAP,
 ...
 * another route. */
#define MYNAH_ASR_PREFORK_QUEUE_UNBOUNDED (-1)
...
#define MYNAH_ASR_PREFORK_QUEUE_NONE      (-2)
...
typedef struct {
     ...
     * through the environment (see mynah_asr_prefork_apply_env). ---- */

    /* Rung 2. Queued connections allowed per LIVE worker, on top of its slots.
     * 0 = unset -> MYNAH_ASR_PREFORK_QUEUE_DEFAULT (1, the reference's default).
     * MYNAH_ASR_PREFORK_QUEUE_UNBOUNDED = no bound, and a shouted warning. */
    ...
    /* ---- DECODER LANE (not implemented here yet). 0 = off, and off is the default. ----
     *
     ...
     *
     * MYNAH_ASR_LANE_SPLIT overrides it; --decoder-lane N is the flag. */
    int lane_cpus;

    /* Preconditions (see mynah_asr_prefork_run). Set by a caller that knows it has
 ...
 * ALLOWED to use, and publishes it as MYNAH_ASR_THREADS so the shared pool -- whose
 ...
 * is a separate call rather than part of mynah_asr_prefork_run(): by the time the
 ...
 * model already open and no inference yet performed, and before any other
 * thread in this process exists.
 *
 * In a child: closes the listening socket, pins to its core slice, rebuilds
 * the thread pool, publishes its worker index, and returns MYNAH_ASR_PREFORK_CHILD
 ...
 * MYNAH_ASR_PREFORK_PARENT_DONE. The parent never runs inference, and the only thing
...
int mynah_asr_prefork_worker_index(void);
 ...
 * MYNAH_ASR_PREFORK_REFUSE_SERVICE_CAP, which is what makes the cap visible in the
 ...
 *
 * What the WORKER must do, and what is still missing: the inference loop has
...
int mynah_asr_prefork_service_cap_ms(void);

/* Applies the MYNAH_ASR_PREFORK_* environment overrides to `cfg` and validates it,
 * shouting about anything dangerous. Called by mynah_asr_prefork_run() and by
 * mynah_asr_prefork_print_plan(); exposed so a caller can resolve the ladder
 ...
 *
 *   MYNAH_ASR_PREFORK_QUEUE       rung 2, queued connections per worker.
