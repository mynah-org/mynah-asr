...
#       [--abort-pct P] [--abort-seed S] [--lang auto|fr|en|...]
#                             the language every client asks for (default auto);
#                             the unloaded reference and the soak use the same.
...
LADDER="8 16 24 32"; SOAK_C=16; SOAK_S=1800; SOAKS=2; ABORT_PCT=""; ABORT_SEED=""; LANG_Q=auto
...
say "            topology ${W}x${T} cap=$CAP  server_cpus=$SERVER_CPUS  gen_cpus=$GEN_CPUS  lang=$LANG_Q"
...
load() { taskset -c "$GEN_CPUS" python3 tools/bench/stream_load.py --port "$PORT" \
             --lookahead "$LOOKAHEAD" --lang "$LANG_Q" --bank "$CLASSES" --class-bounds 8,20 \
